...
import { z } from "zod";
import { requireSnippetOwner } from "../middleware/auth.js";
...
router.post("/", async (req, res) => {
  try {
    ...
    const [id] = await db("snippets").insert({ ...data, user_id: req.user.id });
...
// PUT /api/snippets/:id
router.put("/:id", requireSnippetOwner, async (req, res) => {
  try {
...
router.delete("/:id", requireSnippetOwner, async (req, res) => {
  try {
