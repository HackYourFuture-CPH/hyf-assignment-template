...
const shortTitleMovies = movies.filter(movie => movie.title.length < short);
const longTitleMovies = movies.filter(movie => movie.title.length > long);
...
const ratingSummary = movies.reduce(
  (acc, movie) => {
    if (movie.rating >= 7) acc.goodMovies++;
    else if (movie.rating >= 4) acc.averageMovies++;
    else acc.badMovies++;
    return acc;
  },
...
window.onload = () => {
  renderList("short-titles", "Short Title Movies", shortTitleMovies);
  renderList("long-titles", "Long Title Movies", longTitleMovies);
  renderList("movies-80s", "Movies from the 80s", movies80s);
  renderList("tagged-movies", "Movies with Tags", moviesWithTag);
  renderList("ratings-above-6", "Ratings Above 6", ratingAbove6);
  renderList("duplicate-words", "Movies with Duplicate Words", moviesWithDuplicateWords);

  document.getElementById("keyword-count").innerHTML =
    `<h2>Keyword Count</h2><p>${keyWordCount}</p>`;

  document.getElementById("average-rating").innerHTML =
    `<h2>Average Rating</h2><p>${averageRating.toFixed(2)}</p>`;

  document.getElementById("rating-summary").innerHTML =
    `<h2>Rating Summary</h2>
     <p>Good: ${ratingSummary.goodMovies}</p>
     <p>Average: ${ratingSummary.averageMovies}</p>
     <p>Bad: ${ratingSummary.badMovies}</p>`;
};
